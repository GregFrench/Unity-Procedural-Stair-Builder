...
public class Circle : MonoBehaviour
{
    ...
    void buildMesh()
    {
        int vertices_length = 20 * numSides + 4;
        int normals_length = 20 * numSides + 4;
        int triangles_length = 30 * numSides + 6;
        int uv_length = 20 * numSides + 4;
        int step;
        ...
        step = 20;
        for (int i = 0, count = 0; i < (numSides * step) - 4; i += step, count++)
        {

            degrees = 180 - ((angleOfCurve / numSides) * count);

            pos = getStartPosVectorFromAngle(degrees);
            pos_start = pos * innerRadius;
            pos_end = pos_start + (pos * outerRadius);

            coord_0 = new Vector3(pos_end.x, height, pos_end.z);
            coord_1 = new Vector3(pos_start.x, height, pos_start.z);

            vertices[i + 0] = coord_0;
            vertices[i + 1] = coord_1;

            degrees = 180 - ((angleOfCurve / numSides) * (count + 1));

            pos = getStartPosVectorFromAngle(degrees);
            pos_start = pos * innerRadius;
            pos_end = pos_start + (pos * outerRadius);

            coord_2 = new Vector3(pos_end.x, height, pos_end.z);
            coord_3 = new Vector3(pos_start.x, height, pos_start.z);

            vertices[i + 2] = coord_2;
            vertices[i + 3] = coord_3;

            // front
            vertices[i + 4] = new Vector3(coord_0.x, height * count, coord_0.z);
            vertices[i + 5] = new Vector3(coord_1.x, height * count, coord_1.z);
        ...
        for (int i = 0, count = 0; i < vertices.Length - 4; i += step, count += 1)
        {
            ...
            uv[5 + i] = new Vector2(outerRadius, 0);
            uv[6 + i] = new Vector2(0, height);
            uv[7 + i] = new Vector2(outerRadius, height);
